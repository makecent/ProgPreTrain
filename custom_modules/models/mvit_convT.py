...
class MultiScaleAttention(nn.Module):
    ...
    def forward(
        ...
        q = torch.cat([cls_q.unsqueeze(dim=3).repeat([1, 1, Tq, 1, 1]), q.view(B, self.num_heads, Tq, Hq*Wq, C // self.num_heads)], dim=3)
        k = torch.cat([cls_k.unsqueeze(dim=3).repeat([1, 1, Tk, 1, 1]), k.view(B, self.num_heads, Tk, Hk*Wk, C // self.num_heads)], dim=3)
        v = torch.cat([cls_v.unsqueeze(dim=3).repeat([1, 1, Tv, 1, 1]), v.view(B, self.num_heads, Tv, Hv*Wv, C // self.num_heads)], dim=3)
...
class MultiScaleBlock(nn.Module):
    ...
    def forward(
        ...
        Args:
            ...
            thw_shape (List): The shape of the input tensor (before flattening).
        """
        # Spatial Interaction (self-attention)
        ...
        # Temporal Interaction (convolution)
        x_res = x
        T, H, W = thw_shape_new
        x = rearrange(x, 'b (t hw) c -> (b hw) c t', t=T, hw=H*W)
        x = self.dwconv(x)
        x = rearrange(x, '(b hw) c t -> (b hw) t c', t=T, hw=H*W)
        x = self.norm(x)
        x = F.gelu(self.pwconv1(x))

        x = rearrange(x, '(b hw) t c -> (b hw) c t', t=T, hw=H*W)
        x = self.normt(x)
        x = F.gelu(self.twconv2(x))
        x = rearrange(x, '(b hw) c t -> (b t) hw c', hw=197)
