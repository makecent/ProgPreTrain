...
class WindowAttention3D(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        q = q.reshape(B_, self.num_heads, T, H * W, C // self.num_heads)
        k = k.reshape(B_, self.num_heads, T, H * W, C // self.num_heads)
        v = v.reshape(B_, self.num_heads, T, H * W, C // self.num_heads)
        ...
        x = (attn @ v).movedim(1, 3).reshape(B_, N, C)
        ...
        q = q * self.scale
        T, H, W = self.window_size
        q = q.reshape(B_, self.num_heads, T, H * W, C // self.num_heads).transpose(2, 3)
        k = k.reshape(B_, self.num_heads, T, H * W, C // self.num_heads).transpose(2, 3)
        v = v.reshape(B_, self.num_heads, T, H * W, C // self.num_heads).transpose(2, 3)
        attn = q @ k.transpose(-2, -1)
...
@BACKBONES.register_module(name='SwinTransformer2plus1D')
